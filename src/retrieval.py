...
class VectorRetriever:
    ...
    def retrieve_by_company_name(self, company_name: str, query: str, llm_reranking_sample_size: int = None, top_n: int = 3, return_parent_pages: bool = False) -> List[Tuple[str, float]]:
        ...
        for report in self.all_dbs:
            ...
            if metainfo.get("company_name") == company_name:
                ...
                target_report = report
                break
            # 添加额外的匹配规则：如果文件名包含公司名关键词
            elif "中芯国际" in company_name and "中芯国际" in metainfo.get("file_name", ""):
                target_report = report
                break
            # 添加更通用的匹配规则：检查文件名是否包含目标公司名的关键词
            elif "中芯国际" in metainfo.get("file_name", "") and "中芯国际" in company_name:
                target_report = report
                break
        ...
        faiss_path = self.vector_db_dir / f"{safe_sha1}.faiss"
        if not faiss_path.exists():
    ...
    def _sanitize_filename(self, filename: str) -> str:
        ...
        # 使用更严格的清理方法，只保留字母、数字、下划线和连字符
        import re
        # 移除或替换所有非ASCII字符和特殊符号
        filename = re.sub(r'[^\w\-_]', '_', filename)
        # 限制文件名长度
        if len(filename) > 50:  # 进一步缩短文件名
