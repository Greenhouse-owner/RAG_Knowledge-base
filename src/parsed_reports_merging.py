    ...
    def export_to_markdown(self, reports_dir: Path, output_dir: Path):
        ...
        for report_path in reports_dir.glob("*.json"):
            ...
            if "content" in report_data:
                ...
                # 如果content是页面列表（未处理的报告）
                elif isinstance(content, list):
                    for page in content:
                        if isinstance(page, dict) and "page" in page and "content" in page:
                            document_text += f"\n\n---\n\n# Page {page['page']}\n\n"
                            # 合并页面内容
                            page_texts = [block.get("text", "") for block in page["content"] if isinstance(block, dict)]
                            document_text += "\n".join(page_texts)
