...
class PDFParser:
    def __init__(
        ...
        self.debug_data_path: Path = None
    ...
    def _parse_csv_metadata(csv_path: Path) -> dict:
        ...
        try:
            ...
            print(f'警告：{csv_path.name} 不是 utf-8 编码，自动尝试 gbk 编码...')
    ...
    def convert_documents(self, input_doc_paths: List[Path]) -> Iterable[ConversionResult]:
        try:
            ...
            _log.error(f"Error during document conversion: {str(e)}")
            # 返回一个空的结果集，以便程序可以继续执行
            return []
    ...
    def process_documents(self, conv_results: Iterable[ConversionResult]):
        ...
        for conv_res in conv_results:
            if conv_res.status == ConversionStatus.SUCCESS:
                ...
                failure_count += 1
                _log.info(f"Document {conv_res.input.file} failed to convert. Status: {conv_res.status}")
                _log.info(f"Error details: {conv_res.errors}")
    ...
    def parse_and_export(self, input_doc_paths: List[Path] = None, doc_dir: Path = None):
        ...
        if failure_count > 0:
            ...
            # 不再抛出异常，而是继续处理其他文件
            _log.warning("Continuing with successfully converted documents...")
